...
void group_spot_info::remove(mob* m){
	...
	if(n_current_wheel_members == 1 && current_wheel == mob_wheel){
		...
		unsigned picked_replacement;
		unsigned n_spots = mobs_in_spots[current_wheel].size();
		do{
			picked_replacement = random(0, n_spots-1);
		}while(!mobs_in_spots[current_wheel][picked_replacement]);

		mobs_in_spots[mob_wheel][mob_spot] = mobs_in_spots[current_wheel][picked_replacement];
		mobs_in_spots[current_wheel][picked_replacement] = NULL;
