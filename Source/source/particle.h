...
public:
    ...
    particle(
        ...
        const float friction, const float gravity,
        const float duration, const float size, const ALLEGRO_COLOR &color
    );
    bool tick();
};
...
void random_particle_spray(
    const unsigned char type, ALLEGRO_BITMAP* const bmp,
    const float origin_x, const float origin_y,
    const float angle, const float distance_range, const float angle_range,
