...
void area_editor::delete_mobs(const set<mob_gen*> &which) {
    for(auto &sm : which) {
        ...
        new_mrmi.reserve(game.cur_area_data.mission.team_data[0].goal_mob_idxs.size());
        for(size_t m2 : game.cur_area_data.mission.team_data[0].goal_mob_idxs) {
            if(m2 > m_idx) {
        ...
        game.cur_area_data.mission.team_data[0].goal_mob_idxs = new_mrmi;
...
void area_editor::resize_everything(const float mults[2]) {
    ...
    game.cur_area_data.mission.team_data[0].goal_exit_center.x *= mults[0];
    game.cur_area_data.mission.team_data[0].goal_exit_center.y *= mults[1];
    game.cur_area_data.mission.team_data[0].goal_exit_size.x *= mults[0];
    game.cur_area_data.mission.team_data[0].goal_exit_size.y *= mults[1];
}
