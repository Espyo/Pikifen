...
mob* group_task_category::create_mob(
    ...
    game.gameplay_state->mobs.group_task.push_back(m);
    return m;
}
...
void group_task_category::erase_mob(mob* m) {
    game.gameplay_state->mobs.group_task.erase(
        find(game.gameplay_state->mobs.group_task.begin(), game.gameplay_state->mobs.group_task.end(), (group_task*) m)
    );
}
