 ...
 */
void mob_action::run(mob* m, size_t* action_nr, void* custom_data_1, void* custom_data_2) {
 ...
 * custom_data_1: custom argument #1 to pass to the code.
 */
void mob_fsm::run_event(const size_t type, void* custom_data_1, void* custom_data_2) {
...
mob_event::mob_event(data_node* d, vector<mob_action*> a) :
    ...
    else if(n == "on_leave_hazard")      type = MOB_EVENT_LEFT_HAZARD;
    ...
    for(size_t s = 0; s < states.size(); ++s) {
        ...
        for(size_t e = 0; e < N_MOB_EVENTS; ++e) {
            ...
            for(size_t a = 0; a < ev->actions.size(); ++a) {
                ...
                if(action->type == MOB_ACTION_CHANGE_STATE && !action->vs.empty()) {
