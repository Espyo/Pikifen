...
frame::frame(
    ...
    const float gw, const float gh, const vector<hitbox_instance> &h
) :
    name(name),
    bitmap(b),
    game_w(gw),
...
/* ----------------------------------------------------------------------------
 * Creates a frame of animation, and creates its sprite using a
   * parent bitmap and its coordinates.
 * name:   Internal name, should be unique.
 * b:      Parent bitmap.
...
frame_instance::frame_instance(
    const string &fn, const size_t fnr, frame* fp, const float d
) :
    frame_name(fn),
    frame_nr(fnr),
    frame_ptr(fp),
 ...
 */
animation::animation(
    const string &name, vector<frame_instance> frame_instances,
...
bool animation_instance::tick(const float time) {
    ...
    while(cur_frame_time > cur_frame->duration && cur_frame->duration != 0) {
        ...
        if(cur_frame_nr >= n_frames) {
            done_once = true;
            cur_frame_nr =
                (anim->loop_frame >= n_frames) ? 0 : anim->loop_frame;
        }
...
animation_pool::animation_pool(
    vector<animation*> a, vector<frame*> f, vector<hitbox*> h
) :
    animations(a),
    frames(f),
    hitboxes(h) {
 ...
 */
void animation_pool::create_conversions(
...
animation_pool load_animation_pool_from_file(data_node* file_node) {
    ...
    for(size_t f = 0; f < n_frames; ++f) {
        ...
        for(size_t h = 0; h < n_hitbox_instances; ++h) {
            ...
            cur_hitbox_instance.height =
                s2f(hitbox_instance_node->get_child_by_name("height")->value);
            cur_hitbox_instance.radius =
                s2f(hitbox_instance_node->get_child_by_name("radius")->value);
            cur_hitbox_instance.hitbox_name =
                hitbox_instance_node->name;
            cur_hitbox_instance.type =
                s2i(hitbox_instance_node->get_child_by_name("type")->value);
            cur_hitbox_instance.multiplier =
                s2f(
                    hitbox_instance_node->get_child_by_name("multiplier")->value
                );
            cur_hitbox_instance.can_pikmin_latch =
                s2b(
                    hitbox_instance_node->get_child_by_name(
                        "can_pikmin_latch"
                    )->value
                );
            cur_hitbox_instance.knockback_outward =
                s2b(hitbox_instance_node->get_child_by_name("outward")->value);
            cur_hitbox_instance.knockback_angle =
                s2f(hitbox_instance_node->get_child_by_name("angle")->value);
            cur_hitbox_instance.knockback =
                s2f(
                    hitbox_instance_node->get_child_by_name(
                        "knockback"
                    )->value
            ...
            vector<string> hazards_strs =
                semicolon_list_to_vector(cur_hitbox_instance.hazards_str);
            for(size_t h = 0; h < hazards_strs.size(); ++h) {
                ...
                if(hazards.find(hazard_name) == hazards.end()) {
                    ...
                    cur_hitbox_instance.hazards.push_back(
                        &(hazards[hazard_name])
                    );
                }
            }
        ...
        ALLEGRO_BITMAP* parent =
            bitmaps.get(
                ...
                frame_node->get_child_by_name("file")
            );
        frame* new_f =
            new frame(
            frame_node->name,
        ...
        new_f->top_visible =
            s2b(
                frame_node->get_child_by_name("top_visible")->value
            );
        ...
        for(size_t f = 0; f < n_frame_instances; ++f) {
            ...
            frame_instances.push_back(
                frame_instance(
                    ...
                    s2f(
                        frame_instance_node->get_child_by_name(
                            "duration"
                        )->value
                    )
                )
