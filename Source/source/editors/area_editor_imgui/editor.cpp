...
const float area_editor_imgui::CROSS_SECTION_POINT_RADIUS = 8.0f;
//Minimum distance between the cursor and something snappable for it to snap.
const float area_editor_imgui::CURSOR_SNAP_DISTANCE = 80.0f;
//The cursor snap for heavy modes updates these many times a second.
const float area_editor_imgui::CURSOR_SNAP_UPDATE_INTERVAL = 0.05f;
//Scale the debug text by this much.
const float area_editor_imgui::DEBUG_TEXT_SCALE = 1.3f;
//Default reference image opacity.
const unsigned char area_editor_imgui::DEF_REFERENCE_ALPHA = 128;
//Maximum number of points that a circle sector can be created with.
const unsigned char area_editor_imgui::MAX_CIRCLE_SECTOR_POINTS = 32;
//Maximum grid interval.
const float area_editor_imgui::MAX_GRID_INTERVAL = 4096;
//Maximum number of texture suggestions.
const size_t area_editor_imgui::MAX_TEXTURE_SUGGESTIONS = 20;
//Minimum number of points that a circle sector can be created with.
const unsigned char area_editor_imgui::MIN_CIRCLE_SECTOR_POINTS = 3;
//Minimum grid interval.
const float area_editor_imgui::MIN_GRID_INTERVAL = 2.0;
//Thickness to use when drawing a mob link line.
const float area_editor_imgui::MOB_LINK_THICKNESS = 2.0f;
//How long to tint the new sector's line(s) red for.
const float area_editor_imgui::NEW_SECTOR_ERROR_TINT_DURATION = 1.5f;
//Thickness to use when drawing a path link line.
const float area_editor_imgui::PATH_LINK_THICKNESS = 2.0f;
//Radius to use when drawing a path preview checkpoint.
const float area_editor_imgui::PATH_PREVIEW_CHECKPOINT_RADIUS = 8.0f;
//Only fetch the path these many seconds after the player stops the checkpoints.
const float area_editor_imgui::PATH_PREVIEW_TIMER_DUR = 0.1f;
//Radius to use when drawing a path stop circle.
const float area_editor_imgui::PATH_STOP_RADIUS = 16.0f;
//Scale the letters on the "points" of various features by this much.
const float area_editor_imgui::POINT_LETTER_TEXT_SCALE = 1.5f;
//Color of a selected element, or the selection box.
const unsigned char area_editor_imgui::SELECTION_COLOR[3] = {255, 215, 0};
//Speed at which the selection effect's "wheel" spins, in radians per second.
const float area_editor_imgui::SELECTION_EFFECT_SPEED = TAU * 2;
//Wait this long before letting a new repeat undo operation be saved.
const float area_editor_imgui::UNDO_SAVE_LOCK_DURATION = 1.0f;
//Minimum distance between two vertexes for them to merge.
const float area_editor_imgui::VERTEX_MERGE_RADIUS = 10.0f;
//Maximum zoom level possible in the editor.
const float area_editor_imgui::ZOOM_MAX_LEVEL_EDITOR = 8.0f;
//Minimum zoom level possible in the editor.
const float area_editor_imgui::ZOOM_MIN_LEVEL_EDITOR = 0.01f;


/* ----------------------------------------------------------------------------
 * Creates a layout drawing node based on the mouse's click position.
 */
area_editor_imgui::layout_drawing_node::layout_drawing_node(
    area_editor_imgui* ae_ptr, const point &mouse_click
) :
 ...
 */
area_editor_imgui::texture_suggestion::texture_suggestion(
    const string &n
) :
    bmp(NULL),
    name(n) {
 ...
 */
void area_editor_imgui::texture_suggestion::destroy() {
 ...
 */
area_editor_imgui::area_editor_imgui() :
 ...
 */
bool area_editor_imgui::are_nodes_traversable(
 ...
 */
void area_editor_imgui::calculate_preview_path() {
    ...
    //TODO set_label_text(frm_paths, "lbl_path_dist", "  Total dist.: " + f2s(d));
}
 ...
 */
void area_editor_imgui::cancel_circle_sector() {
    clear_circle_sector();
    //TODO sub_state = EDITOR_SUB_STATE_NONE;
}
 ...
 */
void area_editor_imgui::cancel_layout_drawing() {
 ...
 */
void area_editor_imgui::cancel_layout_moving() {
 ...
 * and description.
 */
void area_editor_imgui::change_snap_mode(const size_t new_mode) {
    snap_mode = new_mode;
 ...
 */
void area_editor_imgui::check_drawing_line(const point &pos) {
    drawing_line_error = DRAWING_LINE_NO_ERROR;
 ...
 */
void area_editor_imgui::clear_circle_sector() {
 ...
 */
void area_editor_imgui::clear_current_area() {
 ...
 */
void area_editor_imgui::clear_layout_drawing() {
    drawing_nodes.clear();
    drawing_line_error = DRAWING_LINE_NO_ERROR;
}
 ...
 */
void area_editor_imgui::clear_layout_moving() {
 ...
 */
void area_editor_imgui::clear_problems() {
 ...
 */
void area_editor_imgui::clear_selection() {
 ...
 */
void area_editor_imgui::clear_texture_suggestions() {
 ...
 */
void area_editor_imgui::clear_undo_history() {
 ...
 */
void area_editor_imgui::create_area() {
    clear_current_area();
 ...
 */
void area_editor_imgui::delete_selected_mobs() {
 ...
 */
void area_editor_imgui::delete_selected_path_elements() {
 ...
 */
void area_editor_imgui::do_logic() {
    editor::do_logic_pre();
 ...
 */
unsigned char area_editor_imgui::find_problems() {
    problem_sector_ptr = NULL;
    problem_vertex_ptr = NULL;
    problem_shadow_ptr = NULL;
    problem_string.clear();
 ...
 */
void area_editor_imgui::finish_circle_sector() {
    clear_layout_drawing();
 ...
 */
void area_editor_imgui::finish_layout_drawing() {
 ...
 */
void area_editor_imgui::finish_layout_moving() {
    TRIANGULATION_ERRORS last_triangulation_error = TRIANGULATION_NO_ERROR;
 ...
 * the undo history.
 */
void area_editor_imgui::forget_prepared_state(area_data* prepared_state) {
 ...
 */
unordered_set<sector*> area_editor_imgui::get_affected_sectors(
    set<vertex*> &vertexes
) {
    unordered_set<sector*> affected_sectors;
 ...
 */
void area_editor_imgui::get_clicked_layout_element(
 ...
 */
edge* area_editor_imgui::get_closest_edge_to_angle(
 ...
 */
bool area_editor_imgui::get_common_sector(
 ...
 * the two provided.
 */
edge* area_editor_imgui::get_correct_post_split_edge(
 ...
 */
bool area_editor_imgui::get_drawing_outer_sector(sector** result) {
 ...
 */
edge* area_editor_imgui::get_edge_under_point(const point &p, edge* after) {
 ...
 */
float area_editor_imgui::get_mob_gen_radius(mob_gen* m) {
 ...
 * with NULLs.
 */
bool area_editor_imgui::get_mob_link_under_point(
    const point &p,
 ...
 */
mob_gen* area_editor_imgui::get_mob_under_point(const point &p) {
    for(size_t m = 0; m < cur_area_data.mob_generators.size(); ++m) {
        mob_gen* m_ptr = cur_area_data.mob_generators[m];
 ...
 * with NULLs.
 */
bool area_editor_imgui::get_path_link_under_point(
    const point &p,
 ...
 */
path_stop* area_editor_imgui::get_path_stop_under_point(const point &p) {
    for(size_t s = 0; s < cur_area_data.path_stops.size(); ++s) {
        path_stop* s_ptr = cur_area_data.path_stops[s];
 ...
 */
sector* area_editor_imgui::get_sector_under_point(const point &p) {
 ...
 */
vertex* area_editor_imgui::get_vertex_under_point(const point &p) {
    for(size_t v = 0; v < cur_area_data.vertexes.size(); ++v) {
        vertex* v_ptr = cur_area_data.vertexes[v];
 ...
 */
vector<edge_intersection> area_editor_imgui::get_intersecting_edges() {
    vector<edge_intersection> intersections;
 ...
 */
void area_editor_imgui::goto_problem() {
 ...
 */
void area_editor_imgui::handle_line_error() {
 ...
 */
void area_editor_imgui::homogenize_selected_mobs() {
 ...
 * based on the one at the head of the selection.
 */
void area_editor_imgui::homogenize_selected_sectors() {
 ...
 */
void area_editor_imgui::load_area(const bool from_backup) {
    clear_current_area();
 ...
 * Loads a backup file.
 */
void area_editor_imgui::load_backup() {
 ...
 */
void area_editor_imgui::load_reference() {
    data_node file(
 ...
 */
void area_editor_imgui::merge_vertex(
 ...
 */
area_data* area_editor_imgui::prepare_state() {
 ...
 */
void area_editor_imgui::register_change(
 ...
 */
bool area_editor_imgui::remove_isolated_sectors() {
 ...
 */
void area_editor_imgui::resize_everything(const float mult) {
    if(mult == 0) {
 ...
 */
bool area_editor_imgui::save_area(const bool to_backup) {
 ...
 */
void area_editor_imgui::save_backup() {
 ...
 */
void area_editor_imgui::save_reference() {
    string file_name =
        USER_AREA_DATA_FOLDER_PATH + "/" + cur_area_name +
        "/Reference.txt";
 ...
 */
void area_editor_imgui::select_edge(edge* e) {
 ...
 */
void area_editor_imgui::select_sector(sector* s) {
 ...
 */
void area_editor_imgui::select_tree_shadow(tree_shadow* s_ptr) {
 ...
 * Selects a vertex.
 */
void area_editor_imgui::select_vertex(vertex* v) {
 ...
 */
void area_editor_imgui::set_new_circle_sector_points() {
    float anchor_angle =
 ...
 * current snap mode.
 */
point area_editor_imgui::snap_point(const point &p) {
 ...
 */
vertex* area_editor_imgui::split_edge(edge* e_ptr, const point &where) {
    point new_v_pos =
        get_closest_point_in_line(
            point(e_ptr->vertexes[0]->x, e_ptr->vertexes[0]->y),
            point(e_ptr->vertexes[1]->x, e_ptr->vertexes[1]->y),
 ...
 */
void area_editor_imgui::start_path_stop_move() {
 ...
 */
void area_editor_imgui::start_shadow_move() {
 ...
 */
void area_editor_imgui::start_vertex_move() {
 ...
 */
void area_editor_imgui::undo_layout_drawing_node() {
    drawing_nodes.erase(
 ...
 */
bool area_editor_imgui::update_backup_status() {
 ...
 */
void area_editor_imgui::update_reference(const string &new_file_name) {
 ...
 */
void area_editor_imgui::update_sector_texture(
 ...
 */
void area_editor_imgui::update_texture_suggestions(const string &n) {
...
void area_editor_imgui::hide_all_frames() { }
void area_editor_imgui::change_to_right_frame() { }
void area_editor_imgui::create_new_from_picker(const size_t picker_id, const string &name) { }
void area_editor_imgui::pick(const size_t picker_id, const string &name, const string &category) { }
area_editor_imgui::~area_editor_imgui() { }
