...
const int editor_old::EDITOR_ICON_BMP_PADDING = 1;
//Every icon in the icon bitmap file has this size.
const int editor_old::EDITOR_ICON_BMP_SIZE = 24;
//Time until the next click is no longer considered a double-click.
const float editor_old::DOUBLE_CLICK_TIMEOUT = 0.5f;
//How much to zoom in/out with the keyboard keys.
const float editor_old::KEYBOARD_CAM_ZOOM = 0.25f;
//How long to override the status bar text for, for important messages.
const float editor_old::STATUS_OVERRIDE_IMPORTANT_DURATION = 6.0f;
//How long to override the status bar text for, for unimportant messages.
const float editor_old::STATUS_OVERRIDE_UNIMPORTANT_DURATION = 1.5f;
//How long the unsaved changes warning stays on-screen for.
const float editor_old::UNSAVED_CHANGES_WARNING_DURATION = 3.0f;
//Height of the unsaved changes warning, sans spike.
const int editor_old::UNSAVED_CHANGES_WARNING_HEIGHT = 30;
//Width and height of the unsaved changes warning's spike.
const int editor_old::UNSAVED_CHANGES_WARNING_SPIKE_SIZE = 16;
//Width of the unsaved changes warning, sans spike.
const int editor_old::UNSAVED_CHANGES_WARNING_WIDTH = 150;

/* ----------------------------------------------------------------------------
 * Initializes editor class stuff.
 */
editor_old::editor_old() :
    cur_picker_id(0),
    double_click_time(0),
    frm_picker(nullptr),
    frm_toolbar(nullptr),
 ...
 */
bool editor_old::check_new_unsaved_changes(lafi::widget* caller_widget) {
 ...
 * from a list.
 */
void editor_old::create_picker_frame() {
 ...
 */
void editor_old::create_status_bar() {
    lbl_status_bar =
 ...
 */
void editor_old::create_toolbar_frame() {
    frm_toolbar =
...
void editor_old::custom_picker_cancel_action() { }
 ...
 */
void editor_old::draw_unsaved_changes_warning() {
 ...
 */
void editor_old::emit_status_bar_message(
 ...
 */
void editor_old::generate_and_open_picker(
...
void editor_old::handle_key_char_anywhere(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_key_char_canvas(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_key_down_anywhere(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_key_down_canvas(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_key_up_anywhere(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_key_up_canvas(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_lmb_double_click(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_lmb_down(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_lmb_drag(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_lmb_up(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_mmb_double_click(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_mmb_down(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_mmb_drag(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_mmb_up(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_mouse_update(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_mouse_wheel(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_rmb_double_click(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_rmb_down(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_rmb_drag(const ALLEGRO_EVENT &ev) {}
void editor_old::handle_rmb_up(const ALLEGRO_EVENT &ev) {}
 ...
 */
bool editor_old::is_mouse_in_gui(const point &mouse_coords) const {
    return
    ...
    game.fade_mgr.start_fade(false, [] () {
        ...
        if(game.states.area_editor_old_st->quick_play_area.empty()) {
            ...
            game.states.gameplay_st->area_to_load =
                game.states.area_editor_old_st->quick_play_area;
 ...
 */
void editor_old::populate_picker(const string &filter) {
...
void editor_old::set_angle_picker_angle(
...
void editor_old::set_textbox_text(
 ...
 */
void editor_old::update_canvas_coordinates() {
    canvas_tl.x = 0;
    canvas_tl.y = 40;
 ...
 */
void editor_old::update_status_bar(
 ...
 */
void editor_old::update_transformations() {
 ...
 * in the same spot.
 */
void editor_old::zoom(const float new_zoom, const bool anchor_cursor) {
 ...
 */
bool editor_old::gui_to_var_helper::all_equal() const {
    for(auto &b : bools) {
 ...
 */
void editor_old::gui_to_var_helper::register_bool(
 ...
 */
void editor_old::gui_to_var_helper::register_color(
 ...
 */
void editor_old::gui_to_var_helper::register_float(
 ...
 */
void editor_old::gui_to_var_helper::register_int(
 ...
 */
void editor_old::gui_to_var_helper::register_point(
 ...
 */
void editor_old::gui_to_var_helper::register_string(
 ...
 */
void editor_old::gui_to_var_helper::register_uchar(
 ...
 */
void editor_old::gui_to_var_helper::set_all() {
    for(auto &b : bools) {
        *(b.first) = b.second;
    }
    for(auto &i : ints) {
...
const float editor_old::transformation_controller::HANDLE_RADIUS = 6.0;
const float editor_old::transformation_controller::ROTATION_HANDLE_THICKNESS = 8.0;


/* ----------------------------------------------------------------------------
 * Creates a transformation controller.
 */
editor_old::transformation_controller::transformation_controller() :
    moving_handle(-1),
    size(point(16, 16)),
    angle(0),
 ...
 */
void editor_old::transformation_controller::draw_handles() {
 ...
 * Returns the angle.
 */
float editor_old::transformation_controller::get_angle() const {
    return angle;
}
 ...
 * Returns the center.
 */
point editor_old::transformation_controller::get_center() const {
    return center;
}
 ...
 */
point editor_old::transformation_controller::get_handle_pos(
 ...
 * Returns the size.
 */
point editor_old::transformation_controller::get_size() const {
    return size;
}
 ...
 */
bool editor_old::transformation_controller::handle_mouse_down(const point pos) {
 ...
 * Returns true if handled, false if nothing was done.
 */
bool editor_old::transformation_controller::handle_mouse_move(const point pos) {
    if(moving_handle == -1) {
        return false;
    }
 ...
 */
void editor_old::transformation_controller::handle_mouse_up() {
    moving_handle = -1;
}
 ...
 * Sets the angle.
 */
void editor_old::transformation_controller::set_angle(const float angle) {
    this->angle = angle;
    update();
}
 ...
 * Sets the center.
 */
void editor_old::transformation_controller::set_center(const point &center) {
    this->center = center;
    update();
}
 ...
 * Sets the size.
 */
void editor_old::transformation_controller::set_size(const point &size) {
    this->size = size;
    update();
}
 ...
 * some caches.
 */
void editor_old::transformation_controller::update() {
