...
void myProgram::doSpatialNavigation(EGSpatNav::DIRECTION direction) {
    ...
    for(const auto& item : myProgram.currentGUI.items) {
        ...
        esnInterface.addNewItem((EasySpatNav::ItemId) item, item.center.x, item.center.y, item.size.x, item.size.y);
    }
...
* The library was not made with overlapping GUI items in mind, so expect incorrect behavior in that case.
