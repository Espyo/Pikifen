...
void GameplayState::drawCutsceneMessageBox() {
    ...
    float transitionRatio =
        ...
        (
            1.0f -
            cutsceneMsgBox->transitionTimer / GAMEPLAY::MENU_EXIT_HUD_MOVE_TIME
        );
...
void GameplayState::drawLightingFilter(const Viewport& view) {
    ...
    if(daylightC.a > 0) {
        al_draw_filled_rectangle(
            0, 0, view.windowRect.size.x, view.windowRect.size.y, daylightC
        );
    }
...
void GameplayState::drawPrecipitation() {
    if(
        game.curArea->weatherCondition.precipitationType !=
...
void GameplayState::drawWorldComponents(
    ...
    components.reserve(
        game.curArea->sectors.size() + //Sectors.
