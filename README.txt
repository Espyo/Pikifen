...
===============
    ...
    The reason for its creation was to make it possible for fans to make their own Pikmin fan content and to be able to play on it, by requiring only assets and data, while the engine itself handles all of the more complicated parts like the physics and artificial intelligence. Its most recent version, in alpha, is available for download. The engine can run on Windows, Linux, and Mac. Being a PC game, it is played using a keyboard, mouse, and/or a gamepad.
    ...
    Players can only currently select what area to play on, after which they are taken to that area and are free to play around with no objective. Win conditions, failure conditions, and progression will be added in the future. Since it is still in development, some features may be fully implemented, missing entirely, or working incorrectly.
